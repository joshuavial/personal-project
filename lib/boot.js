...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  console.log(player)
  router.listenForKeyDown(player)
  ...
  // listen to mouse movement and position relative to canvas
  ...
  //   // player mouse aim relative to player posiiton
  //   mouseX -= player.x
  //   mouseY -= player.y
  ...
  const randomlyGenerateBullet = () => {
    ...
    const width = 10 // between 10 and 40
    const id = Math.random()
    const angle = player.aimAngle // Angle from mouse
  ...
  const drawEntity = (entity) => {
    ctx.save() // saves current canvas fill style
    ctx.fillStyle = entity.color
    ctx.fillRect(entity.x - entity.width / 2,
                 entity.y - entity.height / 2,
                 entity.width,
                 entity.height)
    ctx.restore() // restores saved canvas fill style
  }
  ...
  let updater = new Updater(updateEntity, testCollisionEntity)
  ...
  const update = () => {
    ...
    updater.updateEnemies(enemyList, player)
    ...
    for (let key in bulletList) {
      ...
      if (bulletList[key].timer > 75) { // removes bullets if no hits
        console.log('remove')
        toRemove = true
      }
    ...
    for (let key in upgradeList) {
      ...
      if (isColliding) {
        ...
        delete upgradeList[key] // delete upgrade if collision with player
      }
    }
