...
$(() => {
  ...
  const update = () => {
    ...
    frameCount++ // increase the framecount by one
    ...
    if (frameCount % Math.round(25 / player.attackSpeed) === 0) { // every 1 second
    ...
    for (let key in upgradeList) {
      ...
      if (isColliding) {
        ...
        if (upgradeList[key].category === 'attackSpeed') {
          player.attackSpeed += 3 // increase players attack speed by 3
        }
        delete upgradeList[key] // delete one key
      }
    }
    // when player dies reset game
